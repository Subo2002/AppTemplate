...
const StrictFixedBufferAllocator = @This();
...
const max_num_allocs = 1024;

pub fn init(buffer: []u8) StrictFixedBufferAllocator {
    return .{
        .paddings = buffer[0..max_num_allocs],
        .buffer = buffer[max_num_allocs..],
        .end_index = 0,
        .num_allocs = 0,
    };
}
...
pub fn allocator(self: *StrictFixedBufferAllocator) Allocator {
    return .{
        .ptr = self,
        .vtable = &.{
            .alloc = alloc,
...
pub fn threadSafeAllocator(self: *StrictFixedBufferAllocator) Allocator {
    return .{
        .ptr = self,
        .vtable = &.{
...
pub fn ownsPtr(self: *StrictFixedBufferAllocator, ptr: [*]u8) bool {
    return sliceContainsPtr(self.buffer, ptr);
}

pub fn ownsSlice(self: *StrictFixedBufferAllocator, slice: []u8) bool {
...
/// not reversible.
pub fn isLastAllocation(self: *StrictFixedBufferAllocator, buf: []u8) bool {
    return buf.ptr + buf.len == self.buffer.ptr + self.end_index;
}

pub fn alloc(ctx: *anyopaque, n: usize, alignment: mem.Alignment, ra: usize) ?[*]u8 {
    const self: *StrictFixedBufferAllocator = @ptrCast(@alignCast(ctx));
    _ = ra;
    ...
    self.paddings[self.num_allocs] = @intCast(adjust_off);
    self.num_allocs += 1;
...
pub fn resize(
    ...
    assert(self.isLastAllocation(buf));
...
pub fn free(
    ...
    assert(self.isLastAllocation(buf));
    self.end_index -= buf.len + self.paddings[self.num_allocs - 1];
    self.num_allocs -= 1;
}
...
pub fn reset(self: *StrictFixedBufferAllocator) void {
    self.end_index = 0;
    self.num_allocs = 0;
}
...
test StrictFixedBufferAllocator {
    var fixed_buffer_allocator = mem.validationWrap(StrictFixedBufferAllocator.init(test_fixed_buffer_allocator_memory[0..]));
...
test reset {
    ...
    var fba = StrictFixedBufferAllocator.init(buf[0..]);
...
test "reuse memory on realloc" {
    ...
    {
        var fixed_buffer_allocator = StrictFixedBufferAllocator.init(small_fixed_buffer[0..]);
        const a = fixed_buffer_allocator.allocator();
...
test "thread safe version" {
    var fixed_buffer_allocator = StrictFixedBufferAllocator.init(test_fixed_buffer_allocator_memory[0..]);
